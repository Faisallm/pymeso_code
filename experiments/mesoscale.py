...
class Mesoscale:
    ...
    def _section_global_background_grid(self, aggregate):
        ...
        (Ir, Jr, Kr) = min(self.l, x_max // self.e), min(self.m, y_max // self.e), min(self.n, z_max // self.e)
        (ib, jb, kb) = max(0, ir - 1), max(0, jr - 1), max(0, kr - 1)
        (Ib, Jb, Kb) = min(self.l, Ir + 1), min(self.m, Jr + 1), min(self.n, Kr + 1)
...
m = Mesoscale(300, 300, 300, 4, 20, 10000)